...
- ✅ **Sandbox posture (dev)**: agent code execution runs inside the adapter container as non-root with no host mounts; gVisor runsc + per-task ephemeral containers land with the Linux production deploy (M4)
